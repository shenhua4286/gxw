...
class formguide extends admin {
	...
	public function stat() {
		...
		$m_db = pc_base::load_model('sitemodel_field_model');
		$result = $m_db->select(array('modelid'=>$formid), 'field, setting,formtype');
	...
	public function viewList(){
		...
		*/
		...
		if($field){
			...
			$settings=string2array($fields[$field]['setting']);
			$defaultvalue=$settings['defaultvalue'];
			//print_r('b'.$d['string2array']);
			
			//echo 'defaultvalue'.$field.$fields[$field]['setting']['defaultvalue'];
			$where=array($field =>$fields[$field]['setting']['defaultvalue']);
			
			$r = $m_db->get_one(array(), "COUNT(dataid) sum");
			$total = $r['sum'];
			$pages = pages($total, $page, 20);
			$offset = ($page-1)*20;
			
			$sql="select {$field} from gxw_{$tablename} where {$field} != '{$defaultvalue}' and {$field} != '' and ${field} is not null";
			$result=$m_db->query($sql);
			
			$datas=array();
			
			while($l=mysql_fetch_array($result)){
				$l[$field] = str_replace($defaultvalue, '', $l[$field]);
				$datas[]=$l;
			}
			include $this->admin_tpl('formguide_list_detail');
		}else{
	...
	private function sql_split($sql) {
		...
		$sql = str_replace("gxw_form_table", $this->m_db->db_tablepre.'form_'.$this->tablename, $sql);
		$ret = array();
		$num = 0;
