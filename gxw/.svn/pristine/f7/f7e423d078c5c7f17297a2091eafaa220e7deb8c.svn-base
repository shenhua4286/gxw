...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			$where = 'p.catid='.$catid.' AND p.status='.$status;
			//搜索
			if($catid==19){
				$where='p.catid='.$catid;
			}
			if(isset($_GET['start_time']) && $_GET['start_time']) {
				...
				$where .= " AND `p.inputtime` > '$start_time'";
			}
			if(isset($_GET['end_time']) && $_GET['end_time']) {
				$end_time = strtotime($_GET['end_time']);
				$where .= " AND `p.inputtime` < '$end_time'";
			}
			if($start_time>$end_time) showmessage(L('starttime_than_endtime'));
	
			if(isset($_GET['keyword']) && !empty($_GET['keyword'])) {

				if($modelid==12 || $modelid==28 || $modelid==31 || $modelid==14 || $modelid==15){
					$type_array = array('nickname','title','description','username');
					$searchtype = intval($_GET['searchtype']);
					if($searchtype==0) {
						$searchtype = $type_array[$searchtype];
						$keyword = strip_tags(trim($_GET['keyword']));
						$where .= " AND m.$searchtype like '%$keyword%'";
							
					}elseif($searchtype<3){
						$searchtype = $type_array[$searchtype];
						$keyword = strip_tags(trim($_GET['keyword']));
						$where .= " AND p.$searchtype like '%$keyword%'";
					}elseif($searchtype==4) {
			...
			if(isset($_GET['posids']) && !empty($_GET['posids'])) {
				...
				$where .= " AND `p.posids` = '$posids'";
			}
			
			$sql="";
			if($catid =='13'){
				$sql="select p.*,pj.title projectTitle,m.nickname from gxw_project_progress p left join gxw_project pj on p.projectId=pj.id left join gxw_member m on p.username=m.username where " . $where;
			}else if($catid==29){
				$sql="select * from ".$this->db->table_name." p where ".$where;
			}else{
				$sql="select p.*,m.nickname from ".$this->db->table_name." p left join gxw_member m on p.username=m.username where " . $where;
			}
			//echo $sql;
			$datas = $this->db->mylistinfo($sql,'p.id desc',$_GET['page']);
			...
			for($i=1;$i<=$workflow_steps;$i++) {
				...
				//if($steps == $i){
					...
					$r = $this->db->count(array('catid'=>$catid,'status'=>$i));
					//$newimg = $r ? '<img src="'.IMG_PATH.'icon/new.png" style="padding-bottom:2px" onclick="window.location.href=\'?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.$pc_hash.'\'">'.$r : '';
					$newimg="(".$r.")";
			...
			if($workflow_menu) {
				...
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&pc_hash='.$pc_hash.'&reject=1" '.$current.' ><em>'.'审核未通过'.'</em></a><span>|</span>';
			}
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
				...
				if(isset($_POST['dosubmit'])) {
					...
					showmessage(L('update_success'),'index.php?m=content&c=content&a=init&isShowLeftMenu=1&catid='.$catid);
				}
			} else {
	...
	public function deal(){
		...
		$application_db = pc_base::load_model('info_application_model');
		$admin_username = param::get_cookie('admin_username');
		$catid = intval($_GET['catid']);
		$modelid = $this->categorys[$catid]['modelid'];
		$this->db->set_model($modelid);		
		...
		if(!empty($_GET["reject"])){
			$status = 0;
		}else if(!empty($_GET["agreen"])){
			$status =1;
		}
		switch ($status){
			case 0:
				$title="您修改企业信息的申请被管理员".$admin_username."驳回了";
				break;
			case 1:
				$title="您修改企业信息的申请已通过";
				break;
		...
		if (isset($_POST['reject_c']) && $_POST['reject_c'] != L('reject_msg')) {
			$content = $_POST["reject_c"];
		}else{
			$content='请<a href="index.php?menu=1&m=member&c=index&a=account_manage_info&type=basic&isEdits=1"><font color="red">点击此处</font></a>进行修改';
		}
		if (isset($_POST['ids']) && !empty($_POST['ids'])) {
			foreach ($_POST['ids'] as $id) {
				...
				$application_db->update(array("status"=>$status),array("id"=>$id));
			}
		}
		if($_GET['agreen']){
			showmessage('消息发送成功',HTTP_REFERER);
		}
		showmessage(L('operation_success'),HTTP_REFERER);
	}
	/**
						...
						if ($t=='content') {
							...
							foreach ($model as $modelid) {
								...
								if ($r = $db->count()) { //判断模型下是否有数据
									...
									$db->query('TRUNCATE TABLE `gxw_'.$model_arr[$modelid]['tablename'].'`');
									//开始清理模型data表数据
									...
									$db->query('TRUNCATE TABLE `gxw_'.$model_arr[$modelid]['tablename'].'_data`');
									//删除该模型中在hits表的数据
							...
							for($i=1;;$i++) {
								...
								if ($comment_db->table_exists(str_replace($comment_db->db_tablepre, '', $comment_db->table_name))) {
									if ($r = $comment_db->count()) {
										...
										$comment_db->query('TRUNCATE TABLE `gxw_comment_data_'.$i.'`');
									}
								} else {
									break;
								}
							...
							if ($r = $db->count()) {
								...
								$db->query('TRUNCATE TABLE `gxw_'.$t.'`');
							}
						}
					}
				}
