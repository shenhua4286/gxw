...
if (module_exists('special')) {
	...
	if( !$special_db->field_exists('aid') ){
		...
		$special_db->query("ALTER TABLE  `gxw_special` ADD  `aid` INT UNSIGNED NOT NULL DEFAULT  '0' AFTER  `siteid`");
	}
	if (!$special_db->field_exists('isvideo')) {
		$special_db->query("ALTER TABLE `gxw_special` ADD `isvideo` TINYINT( 1 ) UNSIGNED NOT NULL");
	}
	$special_content_db = pc_base::load_model('special_content_model');
	if (!$special_content_db->field_exists('videoid')) {
		$special_content_db->query("ALTER TABLE `gxw_special_content` ADD `videoid` INT UNSIGNED NOT NULL DEFAULT  '0'");
	}
}
