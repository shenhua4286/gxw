	...
	public function status($ids = array(), $status = 99) {
		...
		if(is_array($ids) && !empty($ids)) {
			foreach($ids as $id) {
				...
				//退稿发送短消息、邮件
				if($r[catid]==10){
					$message = "您提交的产品没有通过审核，请重新编辑提交！<BR>标题：".$r['title']."<BR><a href=\'index.php?m=member&c=content&a=edit&catid={$r[catid]}&id={$r[id]}\'><font color=red>".L('click_edit')."</font></a><br>";									
				}else{
				...
				if(isset($_POST['reject_c']) && $_POST['reject_c'] != L('reject_msg')) {
					...
					if($r['catid']==10){
						$subject = "您的提交的产品被退回，请修改后重新提交";
						$this->message_db->add_message($r['username'],'SYSTEM',$subject,$message);
