...
class content_form {
	...
	function title($field, $value, $fieldinfo) {
		...
		if(defined('IN_ADMIN')) $str .= '<input  style="display: none" type="button" class="button" id="check_title_alt" value="'.L('check_title','','content').'" onclick="$.get(\'?m=content&c=content&a=public_check_title&catid='.$this->catid.'&sid=\'+Math.random()*5, {data:$(\'#title\').val()}, function(data){if(data==\'1\') {$(\'#check_title_alt\').val(\''.L('title_repeat').'\');$(\'#check_title_alt\').css(\'background-color\',\'#FFCC66\');} else if(data==\'0\') {$(\'#check_title_alt\').val(\''.L('title_not_repeat').'\');$(\'#check_title_alt\').css(\'background-color\',\'#F8FFE1\')}})" style="width:73px;"/><img style="display: none" src="'.IMG_PATH.'icon/colour.png" width="15" height="16" onclick="colorpicker(\''.$field.'_colorpanel\',\'set_title_color\');"/> 
		<img style="display: none" src="'.IMG_PATH.'icon/bold.png" width="10" height="10" onclick="input_font_bold()" /> <span id="'.$field.'_colorpanel" style="position:absolute;" class="colorpanel"></span>';
		return $str;
	}
	...
	function images($field, $value, $fieldinfo) {
		...
		if($value) {
			...
			if(is_array($value)) {
				...
				$list_str .= "<div id='image_{$field}_{$_k}' style='padding:1px;float:left;width:124px;position:relative;margin:5px;'><img  style='width:124px;' src='{$_v[url]}'/><input type='hidden' name='{$field}_url[]' value='{$_v[url]}' style='width:310px;' ondblclick='image_priview(this.value);' class='input-text'> <input type='hidden' name='{$field}_alt[]' value='{$_v[alt]}' style='width:160px;' class='input-text'> <a style='width:124px;position:absolute;bottom:0;left:0;display:block;background:#000;height:24px;text-align:center;' href=\"javascript:remove_div('image_{$field}_{$_k}')\">".L('remove_out', '', 'content')."</a></div>";
				}
			}
		} else {
		...
		<fieldset class="blue field-img pad-10" style="margin-left:155px;">
		...
		$string .= $str."<div class='picBut upimgs cu'><a herf='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_images,'{$upload_number},{$upload_allowext},{$isselectimage}','content','$this->catid','{$authkey}')\"/> ".L('select_pic')." </a></div>";
		return $string;
