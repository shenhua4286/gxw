...
class model {
	...
	final public function select($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='') {
		...
		//echo 'ee'.$data.$this->table_name.$where;
	...
	final public function query($sql) {
		$sql = str_replace('gxw_', $this->db_tablepre, $sql);
	 ...
	 * 						为字符串时[例：`name`='gxw',`hits`=`hits`+1]。
	 *						为数组时[例: array('name'=>'gxw','password'=>'123456')]
	...
	public function sql_query($sql) {
		if (!empty($this->db_tablepre)) $sql = str_replace('gxw_', $this->db_tablepre, $sql);
		return $this->db->query($sql);
	}
	...
	//自定义分页查询{支持多表}
	public function mylistinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array()) {
		//echo $where;
		...
		if ($this->number > 0) {
			$sql=$where. ' order by ' .$order .' LIMIT '.$offset.','.$pagesize;
			$this->db->query($sql);
