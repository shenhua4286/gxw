...
class index extends admin {
	private $_filearr = array('api', 'gxw', 'statics', '');
	//md5验证地址
	private $_upgrademd5 = 'http://www.gxw.cn/upgrademd5/';
	//补丁地址
	private $_patchurl = 'http://download.gxw.cn/v9/9.0/patch/';
	...
	public function checkfile() {
		if(!empty($_GET['do'])) {
			...
			//读取gxw接口
			$current_version = pc_base::load_config('version');
			$gxw_md5 = @file_get_contents($this->_upgrademd5.$current_version['pc_release'].'_'.CHARSET.".php");
			$gxw_md5_arr = json_decode($gxw_md5, 1);
			//计算数组差集
			$diff = array_diff($gxw_md5_arr, $this->md5_arr);

			//丢失文件列表
			$lostfile = array();
			foreach($gxw_md5_arr as $k=>$v) {
			...
			//未知文件列表
			$unknowfile = array_diff(array_keys($this->md5_arr), array_keys($gxw_md5_arr));
