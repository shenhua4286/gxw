...
class admin {
	...
	final public static function admin_tpl($file, $m = '') {
		...
		//echo PC_PATH.'modules'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
		//die
	...
	final public static function admin_menu($parentid, $with_self = 0) {
		...
		foreach($result as $v) {
			...
			if(preg_match('/^public_/',$action)) {
				...
				$r = $privdb->get_one(array('m'=>$v['m'],'c'=>$v['c'],'a'=>$action,'data'=>$v['data'],'roleid'=>$_SESSION['roleid'],'siteid'=>$siteid));
				if($r) $array[] = $v;
			}
		}
		return $array;
	...
	final public static function submenu($parentid = '', $big_menu = false) {
		...
		foreach($array as $k=>$_value) {

			if (!isset($_GET['s'])) {
				$classname = ROUTE_M == $_value['m'] && ROUTE_C == $_value['c'] && ROUTE_A == $_value['a'] && $_GET['t']==$k? 'class="on"' : '';
			} else {
				//echo '32';die''
			...
			if($classname) {
				...
				$string .= "<a href='?m=".$_value['m']."&c=".$_value['c']."&t=$k&a=".$_value['a']."&menuid=$parentid&pc_hash=$pc_hash".'&'.$_value['data']."' $classname><em>".L($_value['name'])."</em></a><span>|</span>";
			}
		}
