...
class index extends admin {
	public function __construct() {
		...
		$this->status=array('success'=>false,'msg'=>'');
	}
	...
	public function com_remind(){
		
		$this->member_db = pc_base::load_model('member_model');
		$num = $this->member_db->count("status=2");
	...
	public function public_main() {
		...
		$member_ruzhu=$member_db->count(array('groupid'=>12));	//入住
		$member_shim=$member_db->count(array('groupid'=>12,'status' =>'99'));//实名
		$member_check=$member_ruzhu-$member_shim;//待审核
		
		/***项目信息**/
		$member_db->setTableName("gxw_project");
		$project_num=$member_db->count(array());	//总项目数
		$project_tg=$member_db->count(array("status"=>"99"));	//已通过
		$project_submit=$member_db->count(array("status"=>"0"));	//等待提交
		
		/**产品信息**/
		$member_db->setTableName("gxw_products");
		$products_num=$member_db->count(array());	//总产品数
		
		/**产业园**/
		$member_db->setTableName("gxw_linkage");
		$cyy_num=$member_db->count(array('keyid'=>'3360'));	//总产业园
	...
	public function public_snda_status() {
		...
		if(!strstr(pc_base::load_config('snda','snda_status'), '|')) {
			...
			$uuid = $uuid_arr['uuid'];
			$snda_check_url = "http://open.sdo.com/gxw?cmsid=".$uuid."&sitedomain=".$_SERVER['SERVER_NAME'];
