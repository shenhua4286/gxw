...
<form name="myform" id="myform" action="?m=content&c=content&a=edit" method="post" enctype="multipart/form-data">
	...
	<div class="col-right" <?php if($isShow){?>style="display:none;"<?php }?>>
			<div class="col-1">
	...
	<?php if ($isShow !=1){?>
		...
		<div class="fixed-bottom">
			<div class="fixed-but text-c">
					...
					<input value="<?php if($r['upgrade']) echo $r['url'];?>"
						type="hidden" name="upgrade">
		...
		if ($isShow == 'edit') {
			?>
					<input type="hidden"  name="info[catid]" value="<?php echo $catid;?>"/>
			 		<div class="button">
						<input value="确认编辑" type="submit" name="dosubmit_continue"
							class="cu" onclick="refersh_window()">
					</div>
	
					<div class="button">
						<input value="返回" type="button" name="close"
							onclick="history.go(-1)" class="cu"
							title="Alt+X">
					</div>
		...
		<?php
			...
			?>
				<div class="button">
					<input value="<?php echo L('save_close');?>" type="submit" name="dosubmit"
						class="cu" onclick="refersh_window()">
				</div>
	
				<div class="button">
					<input value="<?php echo L('save_continue');?>" type="submit"
						name="dosubmit_continue" class="cu" onclick="refersh_window()">
				</div>
	
				<div class="button">
					<input value="<?php echo L('c_close');?>" type="button" name="close"
						onclick="refersh_window();close_window();" class="cu" title="Alt+X">
				</div>
