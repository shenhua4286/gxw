...
<tbody>
	...
	?>
		...
		<td align='center'>
			<?php if($r['status']==99){ ?>
				<a class="steps_a" href="javascript:myform.action='?m=content&c=content&a=deal&catid=<?php echo $catid?>&agreen=1';myform.submit();"">同意</a>				
	...
	<?php if($category['content_ishtml']) {?>
		...
		-->	
		<input type="button" class="button" value="同意" onclick="agreen_check();"/>
	   <input type="button" class="button" value="<?php echo L('reject');?>" onclick="reject_check()"/>
		...
		<!--  <div id='agreen_content' style='background-color: #fff;border:#006699 solid 1px;position:absolute;z-index:10;padding:1px;display:none;'>
			<table cellpadding='0' cellspacing='1' border='0'><tr><tr><td colspan='2'><textarea name='agreen_c' id='agreen_c' style='width:300px;height:46px;'  onfocus="if(this.value == this.defaultValue) this.value = ''" onblur="if(this.value.replace(' ','') == '') this.value = this.defaultValue;"><?php echo L('agreen_msg');?></textarea></td><td><input type='button' value=' <?php echo L('submit');?> ' class="button" onclick="agreen_check(1)"></td></tr>
		</table>-->
		</div>		
		<?php }?>
...
function reject_check(type) {
	if(type==1) {
		...
		document.getElementById('myform').action='?m=content&c=content&a=deal&catid=<?php echo $catid;?>&reject=1';
		document.getElementById('myform').submit();
	} else {
		$('#reject_content').css('display','block');
		$('#reject_content').css('position','');
		return false;
	}	
}
function agreen_check() {
	$("input[name='ids[]']").each(function() {
		if($(this).attr('checked')=='checked') {
			str = 1;
		}
	...
	document.getElementById('myform').action='?m=content&c=content&a=deal&catid=<?php echo $catid;?>&agreen=1';
	document.getElementById('myform').submit();	
}
