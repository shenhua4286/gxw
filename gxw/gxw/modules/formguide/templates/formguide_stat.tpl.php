	...
	<?php 
		...
		foreach ($datas AS $__k=>$__v) {
			...
			if($value != $defaultvalue){
				$value=str_replace($defaultvalue, '', $value);
				$number++;
...
function openList(id,field,title) {
	   ...
	   window.top.art.dialog({title:'<?php echo L('stat_formguide')?>--'+title, id:'openList', iframe:'?m=formguide&c=formguide&a=viewList&formid='+id+"&field="+field ,width:'600px',height:'400px'}, function(){window.top.art.dialog({id:'edit'}).close()});
}
