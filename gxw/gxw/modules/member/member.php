...
// 				/$islock = $status == 1 ? 1 : 0;
		...
		$pages = $this->db->pages;
		$big_menu = array("javascript:window.top.art.dialog({id:'add',iframe:'?m=member&c=member&a=add', title:'添加企业', width:'700', height:'500', lock:true}, function(){var d = window.top.art.dialog({id:'add'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'add'}).close()});void(0);", L('member_add'));
	...
	function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$basicinfo['basic_industrys'] = $_POST['info']['basic_industrys'];
			$basicinfo['overduedate'] = strtotime($_POST['info']['overduedate']);
			//'basic_industrys'=>$_POST['info']['basic_industrys']
			//企业基本信息
			...
			$myforminfos=array();
			$type=in_array($_GET['type'], array("basic",'pro','keji')) ? $_GET['type']:'basic';
			//万能字段过滤
			foreach($forminfos as $field=>$info) {
				...
				if(is_numeric(strpos($field,$type))){
					$myforminfos[$field]=$forminfos[$field];
				}
			}
			
			
			//foreach ($forminfos as )
	...
	function check() {
		if(isset($_POST['userid']) && isset($_POST['status'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				foreach ($uidarr as $v){
					//echo $v;
					$user=$this->db->get_one(array('userid'=>$v),'phpssouid');
					$res = $this->client->ps_member_edit('', '', '', '', $user['phpssouid'], '',$status);
	...
	function move() {
		if(isset($_POST['dosubmit'])) {
			...
			$title='';$content='';
			switch($groupid){
				case 10: $content=$title='您的企业资料当前在审核状态'; break;
				case 11: $title='您的企业资料被驳回了,请重新填写资料，进行提交';$content="驳回意见:".$_POST['reject_c']; break;
				case 12: $content=$title='您企业资料已经审核通过了'; break;
				default: $content=$title='您的资料有了新的更新';
			}
	...
	function memberinfo() {
		...
		foreach($model_fieldinfo as $k=>$v) {
			if($v['formtype'] == 'image') {
				$member_modelinfo[$k] = "<a href='$member_modelinfo[$k]' target='_blank'><img src='$member_modelinfo[$k]' height='40' widht='40' ></a>";
			} elseif($v['formtype'] == 'images') {
				$tmp = string2array($member_modelinfo[$k]);
				$member_modelinfo[$k] = '';
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$k] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' ></a>";
					}
					unset($tmp);
				}
