 ...
 *
 * @copyright			(C) 2005-2010 gxw
 * @license				http://www.gxw.cn/license/
 * @lastmodify			2010-6-1
 */
...
function template($module = 'content', $template = 'index', $style = '') {
	...
	$compiledtplfile = gxw_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
	if(file_exists(PC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		...
		$compiledtplfile = gxw_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
...
function my_error_handler($errno, $errstr, $errfile, $errline) {
	if($errno==8) return '';
	$errfile = str_replace(gxw_PATH,'',$errfile);
 ...
 * showmessage('登录成功', array('默认跳转地址'=>'http://www.gxw.cn'));
 ...
 */
function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0,$isSerch) {
	...
	if($datas['style']=='1') {
		...
		$string .= '<script type="text/javascript">
					$(function(){
						...
						setTimeout(\'ld5selected(".pc-select-'.$id.'",'.$default_txt.')\',500);
		...
		if($isSerch){
			$infos['-1']=array('name' =>'全部','linkageid' => '-1');
			asort($infos);
			//array_unshift($infos,array('name' =>'全部','linkageid' => '0'));
		}
		
		$title = $defaultvalue ? $infos[$defaultvalue]['name'] : $datas['title'];
		//print_r("defaultvalue".$defaultvalue.$title);
 ...
 * @param string $font 字体名称 字体请放于gxw\libs\data\font下
...
function p_template($plugin = 'content', $template = 'index',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = pc_base::load_sys_class('template_cache');
	$compiledtplfile = gxw_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.$template.'.php';
